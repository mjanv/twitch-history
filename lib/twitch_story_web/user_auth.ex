defmodule TwitchStoryWeb.UserAuth do
  @moduledoc false

  use TwitchStoryWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_twitch_story_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      TwitchStoryWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_oauth_token(%{assigns: %{current_user: current_user}} = conn, _opts) do
    case Auth.OauthToken.get(current_user) do
      {:ok, token} ->
        ...
        case Auth.OauthToken.expired?(token) do
          true ->
            case Services.Renewal.renew_access_token(current_user) do
  ...
  the current_user:
      ...
      defmodule TwitchStoryWeb.PageLive do
        use TwitchStoryWeb, :live_view

        on_mount {TwitchStoryWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{TwitchStoryWeb.UserAuth, :ensure_authenticated}] do
  ...
  def require_admin_user(conn, _opts) do
    if conn.assigns[:current_user].role == :admin do
      ...
      conn
      |> put_flash(:error, "You must have the administrator role in to access this page.")
      |> redirect(to: ~p"/")
      |> halt()
    end
  end
