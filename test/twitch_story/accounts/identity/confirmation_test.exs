defmodule TwitchStory.Accounts.Identity.ConfirmationTest do
    ...
    test "sends token through notification", %{user: user} do
      token =
        extract_user_token(fn url ->
          Confirmation.deliver_user_confirmation_instructions(user, url)
        end)
  ...
  describe "confirm_user/1" do
    ...
    test "confirms the email with a valid token", %{user: user, token: token} do
      assert {:ok, confirmed_user} = Confirmation.confirm_user(token)
    ...
    test "does not confirm with invalid token", %{user: user} do
      assert Confirmation.confirm_user("oops") == :error
    ...
    test "does not confirm email if token expired", %{user: user, token: token} do
      ...
      assert Confirmation.confirm_user(token) == :error
      refute Repo.get!(User, user.id).confirmed_at
      assert Repo.get_by(UserToken, user_id: user.id)
    end
  end
